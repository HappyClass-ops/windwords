(() => {
  ...
  function closeWordHelper() { if (!els.wordHelper.classList.contains('show')) return; els.wordHelper.classList.remove('show'); [...els.game.children].forEach(el=>{if(el!==els.wordHelper)el.inert=false;}); stopNarration(); voiceRequestToken++; if (showWordHelper.opener?.isConnected) showWordHelper.opener.focus(); }
  ...
  function showWordHelper(word, kind, phase = state.bossMode ? state.bossTargetPhase : state.phase) {
    ...
    showWordHelper.current={word,kind,phase}; speak(word+'. '+kind+'. '+guide+' '+help.join(' '));
  }
  ...
  function updateHud() {
    els.scoreText.textContent = state.score; els.coinText.textContent = state.coins; els.clueText.textContent = "∞"; els.shopCoins.textContent = state.coins; els.lives.innerHTML = "";
    ...
    els.game.classList.toggle("out-of-clues", state.clues <= 0); document.querySelectorAll(".learn-word span").forEach(badge => { badge.textContent = "∞"; }); updateHomeRecord();
  }
  ...
  document.getElementById('helperReplay').addEventListener('click',()=>{ const c=showWordHelper.current; if(c) speak(c.word+'. '+c.kind+'. '+els.helperDefinition.textContent+' '+els.helperExample.textContent); });
  ...
  els.wordHelper.addEventListener('keydown',event=>{ if(event.key!=='Tab')return; const items=[...els.wordHelper.querySelectorAll('button,a[href]')]; const first=items[0],last=items.at(-1); if(event.shiftKey && document.activeElement===first){event.preventDefault();last.focus();} else if(!event.shiftKey && document.activeElement===last){event.preventDefault();first.focus();} });
