(() => {
  ...
  function makeBossOptions(targetPhase, count = 5) { const options=selection.draw(playableBank(targetPhase),enabledKinds(),state.targetKind,count,state.phase===5?3:2);state.bossRemaining=options.filter(o=>o.bossCorrect).length;return options; }
  ...
  async function animatePipTo(island) {
    ...
    if(Math.abs(dx)>1)els.pip.classList.toggle('face-left',dx<0);
  ...
  function startGame() { selection.reset(); PipAdventure.resetRun(); PipAdventure.hideHub(); stopNarration(); state.stageStart=0; sound.ready(); sound.scene("flight"); hideOverlay(els.introOverlay); hideOverlay(els.storyOverlay); hideOverlay(els.resultOverlay); hideOverlay(els.shopOverlay); hideOverlay(els.dictionaryOverlay); closeWordHelper(); state.running = true; state.busy = false; state.score = 0; state.phaseScore = 0; state.phase = state.selectedPhase; state.hearts = 3; state.clues = 3; PipSupplies.reset(PipAdventure.mode); state.streak = 0; state.previousKind = ""; state.bossMode = false; state.bossRemaining = 0; els.game.classList.remove("run-won", "run-lost"); els.choices.innerHTML = ""; els.choices.className = ""; resetAnchor(); applyPhase(state.phase); buildTrail(); const startPosition = getPipPosition(state.anchor); els.pip.style.left = `${startPosition.left}px`; els.pip.style.top = `${startPosition.top}px`; updateHud(); newRound(); }
