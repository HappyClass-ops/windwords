(() => {
  ...
  const state = {
    ...
    dictionaryPhase: 2, stageStart: 0, revealed: new Set()
  };
  ...
  class Sound {
    ...
    toggle() { state.audioOn = !state.audioOn; store.write("audioOn", state.audioOn); if (state.audioOn) this.ready(); else stopNarration(); PipSoundtrack.refresh(); updateSoundButton(); }
  }
  const sound = new Sound();
  PipSoundtrack.configure({enabled:()=>state.audioOn});
  const sleep = ms => new Promise(resolve => setTimeout(resolve, ms));
  PipVoice.configure({enabled:()=>state.audioOn,duck:on=>sound.duck(on)});
  let voiceRequestToken=0;
  const stopNarration=()=>PipVoice.stop();
  ...
  function showWordHelper(word, kind, phase = state.bossMode ? state.bossTargetPhase : PipAdventure.settings(state.phase).wordPhase) {
    ...
    showWordHelper.current={word,kind,phase};
    ...
    if(masked){speak(word+'. Look closely at the picture.');return;}
    ...
    PipMeanings.get(word,kind).then(result=>{
      ...
      document.getElementById('helperOther').textContent=(others.length?'This word can have other jobs too: '+others.join(', ')+'. ':'')+(result?'Meaning: '+result.source+'.':'');
      speak(word+'. '+meaning+' This picture shows a '+kind+'. '+guide);
    });
  }
  ...
  function renderStory() { const details = PHASES[state.selectedPhase]; const wordJobs = state.includeAdverbs ? "nouns, verbs, adjectives and adverbs" : "nouns, verbs and adjectives"; const pages = ['Oh! The islands have floated into the sky, and our friends are stranded. Will you help me bring them home?', 'We will cross the sky islands, floating forest and crystal peaks, all the way to the volcano. Read the words to find a safe path.', 'After six leaps, help me light a beacon. Then choose which path we will take next. Let’s fly!']; els.storyText.textContent = pages[state.storyIndex]; speakPip(pages[state.storyIndex]); els.storyNextButton.textContent = state.storyIndex === pages.length - 1 ? "Let's fly" : "Next"; }
  function beginStory() { PipAdventure.hideHub(); sound.ready(); sound.scene("flight"); state.storyIndex = 0; hideOverlay(els.introOverlay); renderStory(); showOverlay(els.storyOverlay); }
  ...
  function setChoicesDisabled(disabled) { els.choices.querySelectorAll(".island").forEach(island => { island.setAttribute("aria-disabled", String(disabled || island.classList.contains("boss-cleared"))); island.setAttribute("tabindex", disabled || island.classList.contains("boss-cleared") ? "-1" : "0"); }); }
  ...
  function awardCorrect() {
    ...
    const gain = (PipAdventure.mode==='learning'?1:base + hardBonus + streakBonus) * PipAdventure.settings(state.phase).multiplier; state.coins += gain; PipSupplies.earn(gain); state.score += 1; state.best = Math.max(state.best, state.score);
  ...
  async function correctChoice(island) { island.classList.add("correct"); const reward = awardCorrect(); starReward(island,reward.gain); state.phaseScore += 1; sound.correct(); sound.coin(); burstAt(island); climbSky(); updateHud(); [...els.choices.querySelectorAll(".island")].filter(choice => choice !== island).forEach((choice, index) => { choice.animate([{ transform: "translate(0,0)", opacity: 1 }, { transform: `translate(${-135 - index * 22}%, ${42 + index * 13}%)`, opacity: 0 }], { duration: 720, fill: "forwards", easing: "cubic-bezier(.35,.05,.8,.5)" }); }); await sleep(510); await moveNewAnchor(island); await sleep(140); newRound(); }
  async function bossCorrectChoice(island) { island.classList.add("correct", "boss-cleared"); const reward = awardCorrect(); starReward(island,reward.gain); state.bossRemaining -= 1; sound.correct(); sound.coin(); burstAt(island); updateHud(); toast(state.bossRemaining ? `${state.bossRemaining} right island${state.bossRemaining === 1 ? "" : "s"} left` : "Checkpoint cleared!"); await sleep(380); state.anchor = island; island.querySelectorAll("button").forEach(b=>b.disabled=true); if (state.bossRemaining <= 0) await completeCheckpoint(); else { setPrompt(state.targetKind, true); state.busy = false; setChoicesDisabled(false); } }

  async function wrongChoice(island) { state.streak = 0; island.classList.add("wrong"); els.pip.classList.add("panic"); const acceptedKinds = PipVocabulary.kindsFor(island.dataset.word); const uses = acceptedKinds.map(kind => `${article(kind)} ${kind}`).join(" or "); const phrase = `“${island.dataset.word}” can be ${uses}, not ${article(state.targetKind)} ${state.targetKind}.`; toast(phrase, true); els.announcer.textContent = phrase; await sleep(170); for (let i = 1; i <= 3; i++) { island.classList.add(`crack-${i}`); sound.crack(i); await sleep(i === 3 ? 230 : 260); } els.pip.classList.remove("panic"); if (PipSupplies.consumeShield()) { burstAt(island); toast("Cloud shield rescue!"); sound.correct(); await returnPipToAnchor(); island.remove(); state.busy = false; setChoicesDisabled(false); return; } sound.boom(); els.pip.classList.add("fall"); const distance = els.game.clientHeight * .72; const fallPip = els.pip.animate([{ transform: "translateY(0)" }, { transform: `translateY(${distance}px)` }], { duration: 720, fill: "forwards", easing: "cubic-bezier(.5,.1,.8,.7)" }); const fallIsland = island.animate([{ transform: "translateY(0) rotate(0)", opacity: 1 }, { transform: `translateY(${distance * .7}px) rotate(-18deg)`, opacity: 0 }], { duration: 660, fill: "forwards", easing: "ease-in" }); await Promise.all([fallPip.finished, fallIsland.finished]); if(PipAdventure.mode==='classic')state.hearts -= 1; updateHud(); island.remove(); fallPip.cancel(); fallIsland.cancel(); els.pip.classList.remove("fall"); if (state.hearts <= 0) { state.running = false; await sleep(280); showResult(false); } else { await returnPipToAnchor(); state.busy = false; setChoicesDisabled(false); } }
  ...
  function buy(item) { if(!state.running||PipAdventure.mode!=='classic'||!PipSupplies.purchase(item,state))return; sound.purchase();updateHud(); }
  ...
  function renderDictionary() {
    ...
    Object.entries(PHASES[state.dictionaryPhase].words).forEach(([kind, words]) => {
      ...
      matches.forEach(word => { const button = document.createElement("button"); button.type = "button"; button.textContent = word; button.setAttribute("aria-label", `See and hear ${word}, ${kind}`); button.addEventListener("click", () => { sound.clue(); showWordHelper(word,kind,state.dictionaryPhase); }); group.querySelector(".word-chips").appendChild(button); }); els.dictionaryList.appendChild(group);
    });
  ...
  function closeDictionary() { hideOverlay(els.dictionaryOverlay); if (dictionaryReturnOverlay && !state.running) showOverlay(dictionaryReturnOverlay); dictionaryReturnOverlay = null; if(!state.running)PipAdventure.showHub(); groundPip(); }
  ...
  document.getElementById('dictionaryKind').addEventListener('change',renderDictionary);
  document.getElementById('helperReplay').addEventListener('click',()=>{ const c=showWordHelper.current; if(c) speak(c.word+'. '+(els.wordHelper.dataset.kind==='hidden'?'Look closely at the picture.':c.kind+'. '+els.helperDefinition.textContent+' '+els.helperExample.textContent)); });
  ...
  const cosmetics=[{id:'aqua',name:'Aqua glow',stars:15,checkpoints:1,detail:'A turquoise glow around Pip.'},{id:'sunset',name:'Sunset glow',stars:30,checkpoints:3,detail:'A warm golden glow around Pip.'},{id:'chime',name:'Crystal chime',stars:25,checkpoints:2,detail:'A bright extra note for correct answers.'}];
  let ownedCosmetics=store.read('cosmetics',[]); let equippedGlow=store.read('glow',''); let chimeOn=store.read('chime',false);
  function applyCosmetics(){els.pip.dataset.glow=equippedGlow;}
  function renderCosmetics(){const grid=document.getElementById('cosmeticGrid'); grid.replaceChildren(); cosmetics.forEach(item=>{const owned=ownedCosmetics.includes(item.id),locked=state.checkpoints<item.checkpoints,equipped=item.id==='chime'?chimeOn:equippedGlow===item.id; const card=document.createElement('article');card.className='shop-item';card.dataset.cosmetic=item.id;const title=document.createElement('h3');title.textContent=item.name;const detail=document.createElement('p');detail.textContent=item.detail; const button=document.createElement('button');button.className='secondary-button';button.textContent=locked?'Unlock: '+item.checkpoints+' checkpoints':owned?(equipped?'Unequip':'Equip'):'Get · '+item.stars+' stars';button.disabled=locked||(!owned&&state.coins<item.stars);button.onclick=()=>{if(state.checkpoints<item.checkpoints)return;if(!ownedCosmetics.includes(item.id)){if(state.coins<item.stars)return;state.coins-=item.stars;ownedCosmetics.push(item.id);store.write('coins',state.coins);store.write('cosmetics',ownedCosmetics);}if(item.id==='chime'){chimeOn=!chimeOn;store.write('chime',chimeOn);}else{equippedGlow=equippedGlow===item.id?'':item.id;store.write('glow',equippedGlow);}applyCosmetics();sound.purchase();updateHud();updateShop();renderCosmetics();};card.append(title,detail,button);grid.append(card);});}
  const originalCorrect=sound.correct.bind(sound);sound.correct=()=>{originalCorrect();if(chimeOn)sound.tone(1046,.18,'sine',.05,.1);};
  const shopObserver=new MutationObserver(()=>{if(els.shopOverlay.classList.contains('open'))renderCosmetics();});shopObserver.observe(els.shopOverlay,{attributes:true,attributeFilter:['class']});
  applyCosmetics();
  ...
  PipAdventure.attach({stock:()=>{renderCosmetics();const saved=cosmetics.map(i=>{const b=document.querySelector('[data-cosmetic="'+i.id+'"] button');return {...i,cosmetic:true,label:b.textContent,disabled:b.disabled};});return state.running?PipSupplies.stock(state):saved;},purchase:id=>{if(!state.running&&cosmetics.some(i=>i.id===id))document.querySelector('[data-cosmetic="'+id+'"] button')?.click();else buy(id);},supplies:PipSupplies,state,stopVoice:stopNarration,ground:groundPip,refresh:()=>{updatePhasePicker();updateHud();},beginStory,toggleSound:()=>sound.toggle(),openBook:openDictionary,openShop,hideMenus:()=>{closeWordHelper();for(const overlay of [els.introOverlay,els.storyOverlay,els.resultOverlay,els.dictionaryOverlay,els.shopOverlay])hideOverlay(overlay);}});
